...
// Lazily resolved AsyncResource (Node.js only). Avoids top-level await
// so this module can be required from CJS entries as well as imported
// from ESM.
let AsyncResource = null;
function getAsyncResource() {
  if (AsyncResource !== null) return AsyncResource;
  try {
    // Use sync require when available; dynamic import is async.
...
export class NapiRuntime {
  ...
  napi_async_init(args) {
    ...
    const AR = getAsyncResource();
    if (AR) {
      try {
        const ar = new AR(name, { requireManualDestroy: true });
  ...
  // Indirect eval (`(0, eval)`) so the bundle works in environments that
  // disable direct eval (esbuild warning, V8 strict mode). Workerd
  // disables eval entirely; addons that don't call napi_run_script (the
  // common case — oxc-parser, argon2, bcrypt) work fine, this throws at
  // call time only if someone actually invokes it there.
